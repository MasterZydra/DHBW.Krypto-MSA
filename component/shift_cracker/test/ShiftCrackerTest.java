...
public class ShiftCrackerTest {
    ...
    public void crackHalloWieGehtEsIhnen() {
        String decrypted = this.port.decrypt("Lepps ami kilx iw Mlrir", null);
        assertEquals("Hallo wie geht es Ihnen", decrypted);
    }

    @Test
    public void crackEsIstWiederMalSoweit() {
        String decrypted = this.port.decrypt("Ao eop seazan iwh oksaep. Sen sqnzaj cadwygp!", null);
        assertEquals("Es ist wieder mal soweit. Wir wurden gehackt!", decrypted);
    }

    @Test
    public void crackDieMeistenKurzenNachrichten() {
        String decrypted = this.port.decrypt("Nso woscdox uebjox Xkmrbsmrdox gobnox xsmrd uybboud fyw Mbkmuob qoxkmud, nk nso Pboaeoxj fyx k, o, s, y exn e xsmrd yzdswkv scd.", null);
    ...
    public void crackDieCrackbarkeitEinerNachricht() {
        String decrypted = this.port.decrypt("Xcy Wluwevuleycn ychyl Huwblcwbn bähan pih xyl Uhtubf xyl Ogfuony uv.", null);
